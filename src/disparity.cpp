...
class disparity {
    private:
        ...
        // stereo matching object for disparity computation
        int min_disp, num_disp;
        // parameters of StereoSGBM
    public:
        ...
        // show live disparity by processing stereo camera feed
};
...
disparity::disparity(string filename, Size image_size) {
    ...
    stereo = StereoSGBM::create(0, 0, 3, 8*27, 32*27, 1, 63, 0, 100, 32, StereoSGBM::MODE_SGBM);
    /*
    stereo->setPreFilterCap(63);
    //stereo->SADWindowSize(3);
    stereo->setP1(8 * 3 * 3 * 3);
    stereo->setP2(32 * 3 * 3 * 3);
...
void disparity::show_disparity(Size image_size) {
    ...
    while(char(waitKey(1)) != 'q') {
        ...
        // Calculate disparity
        Mat disp, disp_show;
        stereo->compute(framel_rect, framer_rect, disp);
        // Convert disparity to a form easy for visualization
        disp.convertTo(disp_show, CV_8U, 255/(stereo->getNumDisparities() * 16.));
